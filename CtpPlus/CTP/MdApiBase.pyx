...
cdef class MdApiBase:
    ...
    # ############################################################################# #
    #///请求查询组播合约
    def ReqQryMulticastInstrument(self, pQryMulticastInstrument):
        cdef int result = -1
        cdef size_t address = 0
        cdef int nRequestID
        try:
            nRequestID = self.__inc_req_id()
            if self.__api is not NULL:
                address = addressof(pQryMulticastInstrument)
                with nogil:
                    result = self.__api.ReqQryMulticastInstrument(<CThostFtdcQryMulticastInstrumentField *> address, nRequestID)
        except Exception as err_msg:
            self.write_log('ReqQryMulticastInstrument', err_msg)
        finally:
            return result
...
cdef extern  int   MdSpi_OnRspQryMulticastInstrument(self, CThostFtdcMulticastInstrumentField *pMulticastInstrument,
                                                     ...
                                                     bool bIsLast) except -1:
    try:
        self.OnRspQryMulticastInstrument({} if pMulticastInstrument is NULL else MulticastInstrumentField.from_address(<size_t> pMulticastInstrument).to_dict(),
                                         {} if pRspInfo is NULL else RspInfoField.from_address(<size_t> pRspInfo).to_str_dict(),
                                         nRequestID,
                                         bIsLast)
    except Exception as err_msg:
        self.write_log('MdSpi_OnRspQryMulticastInstrument', err_msg)
    return 0
