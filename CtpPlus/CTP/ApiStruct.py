...
from ctypes import *
from CtpPlus.utils.base_field import BaseField
...
class RspUserLoginField(BaseField):
    ...
    _fields_ = [
        ...
        ('SysVersion', c_char * 41),  # 后台版本信息
        ('GFEXTime', c_char * 9)  # 广期所时间
    ]
...
class AuthenticationInfoField(BaseField):
    ...
    _fields_ = [
        ...
        ('AppType', c_char * 1),  # App类型
        ('reserve1', c_char * 16),  # 保留的无效字段
        ('ClientIPAddress', c_char * 33)  # 终端IP地址
    ]
...
class ProductField(BaseField):
    ...
    _fields_ = [
        ...
        ('OpenLimitControlLevel', c_char * 1),  # 开仓量限制粒度
        ('OrderFreqControlLevel', c_char * 1)  # 报单频率控制粒度
    ]
...
class TraderField(BaseField):
    ...
    _fields_ = [
        ...
        ('BrokerID', c_char * 11),  # 经纪公司代码
        ('OrderCancelAlg', c_char * 1),  # 撤单时选择席位算法
        ('TradeInstallCount', c_int),  # 交易报盘安装数量
        ('MDInstallCount', c_int)  # 行情报盘安装数量
    ]
...
class InvestorField(BaseField):
    ...
    _fields_ = [
        ...
        ('IsOrderFreq', c_char * 1),  # 是否频率控制
        ('IsOpenVolLimit', c_char * 1),  # 是否开仓限制
...
class InvestorPositionField(BaseField):
    ...
    _fields_ = [
        ...
        ('InvestUnitID', c_char * 17),  # 投资单元代码
        ('PositionCostOffset', c_double),  # 持仓成本差值
        ('TasPosition', c_int),  # tas持仓手数
...
class DepthMarketDataField(BaseField):
    ...
    _fields_ = [
        ...
        ('BandingUpperPrice', c_double),  # 上带价
        ('BandingLowerPrice', c_double),  # 下带价
    ]
...
class SyncDepositField(BaseField):
    ...
    _fields_ = [
        ...
        ('CurrencyID', c_char * 4),  # 币种代码
        ('IsFromSopt', c_int),  # 是否是个股期权内转
        ('TradingPassword', c_char * 41),  # 资金密码
    ]
...
class InstrumentOrderCommRateField(BaseField):
    ...
    _fields_ = [
        ...
        ('InvestUnitID', c_char * 17),  # 投资单元代码
        ('InstrumentID', c_char * 31),  # 合约代码
        ('OrderCommByTrade', c_double),  # 报单手续费
        ('OrderActionCommByTrade', c_double)  # 撤单手续费
    ]
...
class SyncDelaySwapField(BaseField):
    ...
    _fields_ = [
        ...
        ('IsManualSwap', c_int),  # 是否手工换汇
        ('IsAllRemainSetZero', c_int),  # 是否将所有外币的剩余换汇额度设置为0
    ]
...
class InvestorPositionDetailField(BaseField):
    ...
    _fields_ = [
        ...
        ('CloseVolume', c_int),  # 平仓量
        ('CloseAmount', c_double),  # 平仓金额
        ('TimeFirstVolume', c_int),  # 先开先平剩余数量
        ('InvestUnitID', c_char * 17),  # 投资单元代码
        ('SpecPosiType', c_char * 1),  # 特殊持仓标志
        ('InstrumentID', c_char * 31),  # 合约代码
...
class MulticastInstrumentField(BaseField):
    ...
    _fields_ = [
        ('TopicID', c_int),  # 主题号
        ('reserve1', c_char * 31),  # 保留的无效字段
        ('InstrumentNo', c_int),  # 合约编号
        ('CodePrice', c_double),  # 基准价
        ('VolumeMultiple', c_int),  # 合约数量乘数
        ('PriceTick', c_double),  # 最小变动价位
        ('InstrumentID', c_char * 31)  # 合约代码
    ]
...
class UserSystemInfoField(BaseField):
    ...
    _fields_ = [
        ...
        ('ClientPublicIP', c_char * 33),  # 用户公网IP
        ('ClientLoginRemark', c_char * 151)  # 客户登录备注2
    ]
...
class QryClassifiedInstrumentField(BaseField):
    ...
    _fields_ = [
        ...
        ('ExchangeInstID', c_char * 81),  # 合约在交易所的代码
        ('ProductID', c_char * 31),  # 产品代码
        ('TradingType', c_char * 1),  # 合约交易状态
        ('ClassType', c_char * 1)  # 合约分类类型
    ]
...
class CombPromotionParamField(BaseField):
    ...
    _fields_ = [
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('InstrumentID', c_char * 31),  # 合约代码
        ('CombHedgeFlag', c_char * 5),  # 投机套保标志
        ('Xparameter', c_double)  # 期权组合保证金比例
    ]
...
class RiskSettleProductStatusField(BaseField):
    ...
    _fields_ = [
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('ProductID', c_char * 31),  # 产品编号
        ('ProductStatus', c_char * 1)  # 产品结算状态
    ]
...
class SyncDeltaInfoField(BaseField):
    """风险结算追平信息"""
    _fields_ = [
        ('SyncDeltaSequenceNo', c_int),  # 追平序号
        ('SyncDeltaStatus', c_char * 1),  # 追平状态
        ('SyncDescription', c_char * 257),  # 追平描述
        ('IsOnlyTrdDelta', c_int)  # 是否只有资金追平
    ]
...
class SyncDeltaInvstPosDtlField(BaseField):
    ...
    _fields_ = [
        ...
        ('SettlementID', c_int),  # 结算编号
        ('TradeType', c_char * 1),  # 成交类型
        ('CombInstrumentID', c_char * 13),  # 组合合约代码
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('CloseProfitByDate', c_double),  # 逐日盯市平仓盈亏
        ('CloseProfitByTrade', c_double),  # 逐笔对冲平仓盈亏
        ('PositionProfitByDate', c_double),  # 逐日盯市持仓盈亏
...
class SyncDeltaTradingAccountField(BaseField):
    ...
    _fields_ = [
        ...
        ('SpecProductCloseProfit', c_double),  # 特殊产品平仓盈亏
        ('SpecProductPositionProfitByAlg', c_double),  # 根据持仓盈亏算法计算的特殊产品持仓盈亏
        ('SpecProductPositionProfitByAlg', c_double),  # 特殊产品交易所保证金
        ('SpecProductPositionProfitByAlg', c_double),  # 延时换汇冻结金额
        ('SpecProductPositionProfitByAlg', c_double),  # 剩余换汇额度
        ('SpecProductPositionProfitByAlg', c_int)  # 追平序号
    ]
...
class SyncDeltaInitInvstMarginField(BaseField):
    ...
    _fields_ = [
        ('BrokerID', c_char * 11),  # 经纪公司代码
        ('InvestorID', c_char * 13),  # 投资者代码
        ('LastRiskTotalInvstMargin', c_double),  # 追平前总风险保证金
        ('LastRiskTotalExchMargin', c_double),  # 追平前交易所总风险保证金
        ('ThisSyncInvstMargin', c_double),  # 本次追平品种总保证金
        ('ThisSyncExchMargin', c_double),  # 本次追平品种交易所总保证金
        ('RemainRiskInvstMargin', c_double),  # 本次未追平品种总保证金
        ('RemainRiskExchMargin', c_double),  # 本次未追平品种交易所总保证金
        ('LastRiskSpecTotalInvstMargin', c_double),  # 追平前总特殊产品风险保证金
        ('LastRiskSpecTotalExchMargin', c_double),  # 追平前总特殊产品交易所风险保证金
        ('ThisSyncSpecInvstMargin', c_double),  # 本次追平品种特殊产品总保证金
        ('ThisSyncSpecExchMargin', c_double),  # 本次追平品种特殊产品交易所总保证金
        ('RemainRiskSpecInvstMargin', c_double),  # 本次未追平品种特殊产品总保证金
        ('RemainRiskSpecExchMargin', c_double),  # 本次未追平品种特殊产品交易所总保证金
        ('SyncDeltaSequenceNo', c_int)  # 追平序号
    ]
...
class SyncDeltaDceCombInstrumentField(BaseField):
    """风险结算追平组合优先级"""
    _fields_ = [
        ('CombInstrumentID', c_char * 31),  # 合约代码
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('ExchangeInstID', c_char * 81),  # 合约在交易所的代码
        ('TradeGroupID', c_int),  # 成交组号
        ('CombHedgeFlag', c_char * 1),  # 投机套保标志
        ('CombinationType', c_char * 1),  # 组合类型
        ('Direction', c_char * 1),  # 买卖
        ('ProductID', c_char * 31),  # 产品代码
        ('Xparameter', c_double),  # 期权组合保证金比例
...
class SyncDeltaInvstCommRateField(BaseField):
    ...
    _fields_ = [
        ...
        ('InvestorID', c_char * 13),  # 投资者代码
        ('OpenRatioByMoney', c_double),  # 开仓手续费率
        ('OpenRatioByMoney', c_double),  # 开仓手续费
        ('OpenRatioByMoney', c_double),  # 平仓手续费率
        ('OpenRatioByMoney', c_double),  # 平仓手续费
        ('OpenRatioByMoney', c_double),  # 平今手续费率
        ('OpenRatioByMoney', c_double),  # 平今手续费
        ('OpenRatioByMoney', c_char * 1),  # 操作标志
        ('OpenRatioByMoney', c_int)  # 追平序号
    ]
...
class SPBMFutureParameterField(BaseField):
    ...
    _fields_ = [
        ...
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('InstrumentID', c_char * 31),  # 合约代码
        ('ProdFamilyCode', c_char * 31),  # 品种代码
        ('Cvf', c_int),  # 期货合约因子
        ('TimeRange', c_char * 1),  # 阶段标识
        ('MarginRate', c_double),  # 品种保证金标准
        ('LockRateX', c_double),  # 期货合约内部对锁仓费率折扣比例
        ('AddOnRate', c_double),  # 提高保证金标准
        ('PreSettlementPrice', c_double),  # 昨结算价
        ('AddOnLockRateX2', c_double),  # 期货合约内部对锁仓附加费率折扣比例
    ]
...
class SPBMOptionParameterField(BaseField):
    ...
    _fields_ = [
        ...
        ('ProdFamilyCode', c_char * 31),  # 品种代码
        ('Cvf', c_int),  # 期权合约因子
        ('DownPrice', c_double),  # 期权冲抵价格
        ('Delta', c_double),  # Delta值
        ('SlimiDelta', c_double),  # 卖方期权风险转换最低值
        ('PreSettlementPrice', c_double)  # 昨结算价
    ]
...
class SPBMIntraParameterField(BaseField):
    ...
    _fields_ = [
        ...
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('ProdFamilyCode', c_char * 31),  # 品种代码
        ('IntraRateY', c_double),  # 品种内合约间对锁仓费率折扣比例
        ('AddOnIntraRateY2', c_double)  # 品种内合约间对锁仓附加费率折扣比例
    ]
...
class SPBMInterParameterField(BaseField):
    ...
    _fields_ = [
        ...
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('SpreadId', c_int),  # 优先级
        ('InterRateZ', c_double),  # 品种间对锁仓费率折扣比例
        ('Leg1ProdFamilyCode', c_char * 31),  # 第一腿构成品种
        ('Leg2ProdFamilyCode', c_char * 31)  # 第二腿构成品种
    ]
...
class SPBMPortfDefinitionField(BaseField):
    ...
    _fields_ = [
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('PortfolioDefID', c_int),  # 组合保证金套餐代码
        ('ProdFamilyCode', c_char * 31),  # 品种代码
        ('IsSPBM', c_int)  # 是否启用SPBM
    ]
...
class InvestorPortfMarginRatioField(BaseField):
    ...
    _fields_ = [
        ...
        ('InvestorID', c_char * 13),  # 投资者代码
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('MarginRatio', c_double)  # 会员对投资者收取的保证金和交易所对投资者收取的保证金的比例
    ]
...
class InvestorProdSPBMDetailField(BaseField):
    ...
    _fields_ = [
        ...
        ('InvestorID', c_char * 13),  # 投资者代码
        ('ProdFamilyCode', c_char * 31),  # 品种代码
        ('IntraInstrMargin', c_double),  # 合约内对锁保证金
        ('BCollectingMargin', c_double),  # 买归集保证金
        ('SCollectingMargin', c_double),  # 卖归集保证金
        ('IntraProdMargin', c_double),  # 品种内合约间对锁保证金
        ('NetMargin', c_double),  # 净保证金
        ('InterProdMargin', c_double),  # 产品间对锁保证金
        ('SingleMargin', c_double),  # 裸保证金
        ('AddOnMargin', c_double),  # 附加保证金
        ('DeliveryMargin', c_double),  # 交割月保证金
        ('CallOptionMinRisk', c_double),  # 看涨期权最低风险
        ('PutOptionMinRisk', c_double),  # 看跌期权最低风险
        ('OptionMinRisk', c_double),  # 卖方期权最低风险
        ('OptionValueOffset', c_double),  # 买方期权冲抵价值
        ('OptionRoyalty', c_double),  # 卖方期权权利金
        ('RealOptionValueOffset', c_double),  # 价值冲抵
        ('Margin', c_double),  # 保证金
        ('ExchMargin', c_double)  # 交易所保证金
    ]
...
class PortfTradeParamSettingField(BaseField):
    ...
    _fields_ = [
        ...
        ('BrokerID', c_char * 11),  # 经纪公司代码
        ('InvestorID', c_char * 13),  # 投资者代码
        ('Portfolio', c_char * 1),  # 新型组保算法
        ('IsActionVerify', c_int),  # 撤单是否验资
        ('IsCloseVerify', c_int)  # 平仓是否验资
    ]


class InvestorTradingRightField(BaseField):
    """投资者交易权限设置"""
    _fields_ = [
        ('BrokerID', c_char * 11),  # 经纪公司代码
        ('InvestorID', c_char * 13),  # 投资者代码
        ('InvstTradingRight', c_char * 1)  # 交易权限
    ]


class MortgageParamField(BaseField):
    """质押配比参数"""
    _fields_ = [
        ('BrokerID', c_char * 11),  # 经纪公司代码
        ('AccountID', c_char * 13),  # 投资者帐号
        ('MortgageBalance', c_double),  # 质押配比系数
        ('CheckMortgageRatio', c_int)  # 开仓是否验证质押配比
    ]


class WithDrawParamField(BaseField):
    """可提控制参数"""
    _fields_ = [
        ('BrokerID', c_char * 11),  # 经纪公司代码
        ('AccountID', c_char * 13),  # 投资者帐号
        ('WithDrawParamID', c_char * 1),  # 参数代码
        ('WithDrawParamValue', c_char * 41)  # 参数代码值
    ]


class ThostUserFunctionField(BaseField):
    """Thost终端用户功能权限"""
    _fields_ = [
        ('BrokerID', c_char * 11),  # 经纪公司代码
        ('UserID', c_char * 16),  # 用户代码
        ('ThostFunctionCode', c_int)  # Thost终端功能代码
    ]


class QryThostUserFunctionField(BaseField):
    """Thost终端用户功能权限查询"""
    _fields_ = [
        ('BrokerID', c_char * 11),  # 经纪公司代码
        ('UserID', c_char * 16)  # 用户代码
...
class SPBMAddOnInterParameterField(BaseField):
    """SPBM附加跨品种抵扣参数"""
    _fields_ = [
        ('TradingDay', c_char * 16),  # 交易日
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('SpreadId', c_int),  # 优先级
        ('AddOnInterRateZ2', c_double),  # 品种间对锁仓附加费率折扣比例
        ('Leg1ProdFamilyCode', c_char * 31),  # 第一腿构成品种
        ('Leg2ProdFamilyCode', c_char * 31)  # 第二腿构成品种
    ]


class QrySPBMAddOnInterParameterField(BaseField):
    """SPBM附加跨品种抵扣参数查询"""
    _fields_ = [
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('Leg1ProdFamilyCode', c_char * 31),  # 第一腿构成品种
...
class QryInvestorCommoditySPMMMarginField(BaseField):
    """投资者商品组SPMM记录查询"""
    _fields_ = [
        ('BrokerID', c_char * 11),  # 经纪公司代码
        ('InvestorID', c_char * 13),  # 交易所代码
        ('CommodityID', c_char * 41)  # 商品组代码
    ]


class QryInvestorCommodityGroupSPMMMarginField(BaseField):
    """投资者商品群SPMM记录查询"""
    _fields_ = [
        ('BrokerID', c_char * 11),  # 经纪公司代码
        ('InvestorID', c_char * 13),  # 投资者代码
        ('CommodityGroupID', c_char * 41)  # 商品群代码
    ]


class QrySPMMInstParamField(BaseField):
    """SPMM合约参数查询"""
    _fields_ = [
        ('InstrumentID', c_char * 31)  # 合约代码
    ]
...
class QrySPMMProductParamField(BaseField):
    """SPMM产品参数查询"""
    _fields_ = [
        ('ProductID', c_char * 41)  # 产品代码
    ]


class InvestorCommoditySPMMMarginField(BaseField):
    """投资者商品组SPMM记录"""
    _fields_ = [
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('BrokerID', c_char * 11),  # 经纪公司代码
        ('InvestorID', c_char * 13),  # 投资者代码
        ('CommodityID', c_char * 41),  # 商品组代码
        ('MarginBeforeDiscount', c_double),  # 优惠仓位应收保证金
        ('MarginNoDiscount', c_double),  # 不优惠仓位应收保证金
        ('LongPosRisk', c_double),  # 多头实仓风险
        ('LongOpenFrozenRisk', c_double),  # 多头开仓冻结风险
        ('LongCloseFrozenRisk', c_double),  # 多头被平冻结风险
        ('ShortPosRisk', c_double),  # 空头实仓风险
        ('ShortOpenFrozenRisk', c_double),  # 空头开仓冻结风险
        ('ShortCloseFrozenRisk', c_double),  # 空头被平冻结风险
        ('IntraCommodityRate', c_double),  # SPMM品种内跨期优惠系数
        ('OptionDiscountRate', c_double),  # SPMM期权优惠系数
        ('PosDiscount', c_double),  # 实仓对冲优惠金额
        ('OpenFrozenDiscount', c_double),  # 开仓报单对冲优惠金额
        ('NetRisk', c_double),  # 品种风险净头
        ('CloseFrozenMargin', c_double),  # 平仓冻结保证金
        ('FrozenCommission', c_double),  # 冻结的手续费
        ('Commission', c_double),  # 手续费
        ('FrozenCash', c_double),  # 冻结的资金
        ('CashIn', c_double),  # 资金差额
        ('StrikeFrozenMargin', c_double)  # 行权冻结资金
    ]


class InvestorCommodityGroupSPMMMarginField(BaseField):
    """投资者商品群SPMM记录"""
    _fields_ = [
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('BrokerID', c_char * 11),  # 经纪公司代码
        ...
        ('MarginBeforeDiscount', c_double),  # 优惠仓位应收保证金
        ('MarginNoDiscount', c_double),  # 不优惠仓位应收保证金
        ('LongRisk', c_double),  # 多头风险
        ('ShortRisk', c_double),  # 空头风险
        ('CloseFrozenMargin', c_double),  # 商品群平仓冻结保证金
        ('InterCommodityRate', c_double),  # SPMM跨品种优惠系数
        ('MiniMarginRatio', c_double),  # 商品群最小保证金比例
        ('AdjustRatio', c_double),  # 投资者保证金和交易所保证金的比例
        ('IntraCommodityDiscount', c_double),  # SPMM品种内优惠汇总
        ('InterCommodityDiscount', c_double),  # SPMM跨品种优惠
        ('ExchMargin', c_double),  # 交易所保证金
        ('InvestorMargin', c_double),  # 投资者保证金
        ('FrozenCommission', c_double),  # 冻结的手续费
        ('Commission', c_double),  # 手续费
        ('FrozenCash', c_double),  # 冻结的资金
        ('CashIn', c_double),  # 资金差额
...
class SPMMInstParamField(BaseField):
    """SPMM合约参数"""
    _fields_ = [
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('InstrumentID', c_char * 31),  # 合约代码
        ('InstMarginCalID', c_char * 1),  # SPMM合约保证金算法
        ('CommodityID', c_char * 41),  # 商品组代码
        ('CommodityGroupID', c_char * 41)  # 商品群代码
    ]


class SPMMProductParamField(BaseField):
    """SPMM产品参数"""
    _fields_ = [
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('ProductID', c_char * 41),  # 产品代码
...
class QryTraderAssignField(BaseField):
    """席位与交易中心对应关系维护查询"""
    _fields_ = [
        ('TraderID', c_char * 21)  # 交易员代码
    ]


class TraderAssignField(BaseField):
    """席位与交易中心对应关系"""
    _fields_ = [
        ('BrokerID', c_char * 11),  # 应用单元代码
        ('ExchangeID', c_char * 9),  # 交易所代码
...
class InvestorInfoCntSettingField(BaseField):
    """投资者申报费阶梯收取设置"""
    _fields_ = [
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('BrokerID', c_char * 11),  # 经纪公司代码
        ('InvestorID', c_char * 13),  # 投资者代码
        ('ProductID', c_char * 41),  # 商品代码
        ('IsCalInfoComm', c_int),  # 是否收取申报费
        ('IsLimitInfoMax', c_int),  # 是否限制信息量
        ('InfoMaxLimit', c_int)  # 信息量限制笔数
    ]


class RCAMSCombProductInfoField(BaseField):
    """RCAMS产品组合信息"""
    _fields_ = [
        ('TradingDay', c_char * 9),  # 交易日
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('ProductID', c_char * 41),  # 产品代码
        ('CombProductID', c_char * 41),  # 商品组代码
        ('ProductGroupID', c_char * 41)  # 商品群代码
    ]


class RCAMSInstrParameterField(BaseField):
    """RCAMS同合约风险对冲参数"""
    _fields_ = [
        ('TradingDay', c_char * 9),  # 交易日
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('ProductID', c_char * 41),  # 产品代码
        ('HedgeRate', c_double)  # 同合约风险对冲比率
    ]


class RCAMSIntraParameterField(BaseField):
    """RCAMS品种内风险对冲参数"""
    _fields_ = [
        ('TradingDay', c_char * 9),  # 交易日
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('CombProductID', c_char * 41),  # 产品组合代码
        ('HedgeRate', c_double)  # 品种内对冲比率
    ]


class RCAMSInterParameterField(BaseField):
    """RCAMS跨品种风险折抵参数"""
    _fields_ = [
        ('TradingDay', c_char * 9),  # 交易日
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('ProductGroupID', c_char * 41),  # 商品群代码
        ('Priority', c_int),  # 优先级
        ('CreditRate', c_double),  # 折抵率
        ('CombProduct1', c_char * 41),  # 产品组合代码1
        ('CombProduct2', c_char * 41)  # 产品组合代码2
    ]


class RCAMSShortOptAdjustParamField(BaseField):
    """RCAMS空头期权风险调整参数"""
    _fields_ = [
        ('TradingDay', c_char * 9),  # 交易日
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('CombProductID', c_char * 41),  # 产品组合代码
        ('HedgeFlag', c_char * 1),  # 投套标志
        ('AdjustValue', c_double)  # 空头期权风险调整标准
    ]


class RCAMSInvestorCombPositionField(BaseField):
    """RCAMS策略组合持仓"""
    _fields_ = [
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('BrokerID', c_char * 11),  # 经纪公司代码
        ...
        ('CombInstrumentID', c_char * 31),  # 组合合约代码
        ('LegID', c_int),  # 单腿编号
        ('ExchangeInstID', c_char * 81),  # 交易所组合合约代码
        ('TotalAmt', c_int),  # 持仓量
        ('ExchMargin', c_double),  # 交易所保证金
        ('Margin', c_double)  # 投资者保证金
    ]
...
class InvestorProdRCAMSMarginField(BaseField):
    """投资者品种RCAMS保证金"""
    _fields_ = [
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('BrokerID', c_char * 11),  # 经纪公司代码
        ...
        ('HedgeFlag', c_char * 1),  # 投套标志
        ('ProductGroupID', c_char * 41),  # 商品群代码
        ('RiskBeforeDiscount', c_double),  # 品种组合前风险
        ('IntraInstrRisk', c_double),  # 同合约对冲风险
        ('BPosRisk', c_double),  # 品种买持仓风险
        ('SPosRisk', c_double),  # 品种卖持仓风险
        ('IntraProdRisk', c_double),  # 品种内对冲风险
        ('NetRisk', c_double),  # 品种净持仓风险
        ('InterProdRisk', c_double),  # 品种间对冲风险
        ('ShortOptRiskAdj', c_double),  # 空头期权风险调整
        ('OptionRoyalty', c_double),  # 空头期权权利金
        ('MMSACloseFrozenMargin', c_double),  # 大边组合平仓冻结保证金
        ('CloseCombFrozenMargin', c_double),  # 平策略组合冻结保证金
        ('CloseFrozenMargin', c_double),  # 平仓冻结保证金
        ('MMSAOpenFrozenMargin', c_double),  # 大边组合开仓冻结保证金
        ('DeliveryOpenFrozenMargin', c_double),  # 交割月期货开仓冻结保证金
        ('OpenFrozenMargin', c_double),  # 开仓冻结保证金
        ('UseFrozenMargin', c_double),  # 投资者冻结保证金
        ('MMSAExchMargin', c_double),  # 大边组合交易所持仓保证金
        ('DeliveryExchMargin', c_double),  # 交割月期货交易所持仓保证金
        ('CombExchMargin', c_double),  # 策略组合交易所保证金
        ('ExchMargin', c_double),  # 交易所持仓保证金
        ('UseMargin', c_double)  # 投资者持仓保证金
    ]


class QryRCAMSCombProductInfoField(BaseField):
    """RCAMS产品组合信息查询"""
    _fields_ = [
        ('ProductID', c_char * 41),  # 产品代码
        ('CombProductID', c_char * 41),  # 商品组代码
...
class QryRCAMSInstrParameterField(BaseField):
    """RCAMS同合约风险对冲参数查询"""
    _fields_ = [
        ('ProductID', c_char * 41)  # 产品代码
    ]
...
class QryRCAMSIntraParameterField(BaseField):
    """RCAMS品种内风险对冲参数查询"""
    _fields_ = [
            ('CombProductID', c_char * 41)  # 产品组合代码
        ]
...
class QryRCAMSInterParameterField(BaseField):
    """RCAMS跨品种风险折抵参数查询"""
    _fields_ = [
        ('ProductGroupID', c_char * 41),  # 商品群代码
        ('CombProduct1', c_char * 41),  # 产品组合代码1
...
class QryRCAMSShortOptAdjustParamField(BaseField):
    """RCAMS空头期权风险调整参数查询"""
    _fields_ = [
        ('CombProductID', c_char * 41)  # 产品组合代码
    ]
...
class QryRCAMSInvestorCombPositionField(BaseField):
    """RCAMS策略组合持仓查询"""
    _fields_ = [
        ('BrokerID', c_char * 11),  # 经纪公司代码
        ('InvestorID', c_char * 13),  # 投资者代码
...
class QryInvestorProdRCAMSMarginField(BaseField):
    """投资者品种RCAMS保证金查询"""
    _fields_ = [
        ('BrokerID', c_char * 11),  # 经纪公司代码
        ('InvestorID', c_char * 13),  # 投资者代码
        ('CombProductID', c_char * 41),  # 合约代码
        ('ProductGroupID', c_char * 41)  # 组合合约代码
    ]


class RULEInstrParameterField(BaseField):
    """RULEInstrParameterField"""
    _fields_ = [
        ('TradingDay', c_char * 9),  # 交易日
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('InstrumentID', c_char * 31),  # 合约代码
        ('InstrumentClass', c_char * 1),  # 合约类型
        ('StdInstrumentID', c_char * 31),  # 标准合约
        ('BSpecRatio', c_double),  # 投机买折算系数
        ('SSpecRatio', c_double),  # 投机卖折算系数
        ('BHedgeRatio', c_double),  # 套保买折算系数
        ('SHedgeRatio', c_double),  # 套保卖折算系数
        ('BAddOnMargin', c_double),  # 买附加风险保证金
        ('SAddOnMargin', c_double),  # 卖附加风险保证金
        ('CommodityGroupID', c_int)  # 商品群号
    ]


class RULEIntraParameterField(BaseField):
    """RULE品种内对锁仓折扣参数"""
    _fields_ = [
        ('TradingDay', c_char * 9),  # 交易日
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('ProdFamilyCode', c_char * 31),  # 品种代码
        ('StdInstrumentID', c_char * 31),  # 标准合约
        ('StdInstrMargin', c_double),  # 标准合约保证金
        ('UsualIntraRate', c_double),  # 一般月份合约组合保证金系数
        ('DeliveryIntraRate', c_double)  # 临近交割合约组合保证金系数
    ]


class RULEInterParameterField(BaseField):
    """RULE跨品种抵扣参数"""
    _fields_ = [
        ('TradingDay', c_char * 9),  # 交易日
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('SpreadId', c_int),  # 优先级
        ('InterRate', c_double),  # 品种间对锁仓费率折扣比例
        ('Leg1ProdFamilyCode', c_char * 31),  # 第一腿构成品种
        ('Leg2ProdFamilyCode', c_char * 31),  # 第二腿构成品种
        ('Leg1PropFactor', c_int),  # 腿1比例系数
        ('Leg2PropFactor', c_int),  # 腿2比例系数
        ('CommodityGroupID', c_int),  # 商品群号
        ('CommodityGroupName', c_char * 21)  # 商品群名称
    ]


class QryRULEInstrParameterField(BaseField):
    """RULE合约保证金参数查询"""
    _fields_ = [
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('InstrumentID', c_char * 9)  # 合约代码
    ]


class QryRULEIntraParameterField(BaseField):
    """RULE品种内对锁仓折扣参数查询"""
    _fields_ = [
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('ProdFamilyCode', c_char * 31)  # 品种代码
...
class QryRULEInterParameterField(BaseField):
    """RULE跨品种抵扣参数查询"""
    _fields_ = [
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('Leg1ProdFamilyCode', c_char * 31),  # 第一腿构成品种
...
class InvestorProdRULEMarginField(BaseField):
    """投资者产品RULE保证金"""
    _fields_ = [
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('BrokerID', c_char * 11),  # 经纪公司代码
        ...
        ('InstrumentClass', c_char * 1),  # 合约类型
        ('CommodityGroupID', c_int),  # 商品群号
        ('BStdPosition', c_double),  # 买标准持仓
        ('SStdPosition', c_double),  # 卖标准持仓
        ('BStdOpenFrozen', c_double),  # 买标准开仓冻结
        ('SStdOpenFrozen', c_double),  # 卖标准开仓冻结
        ('BStdCloseFrozen', c_double),  # 买标准平仓冻结
        ('SStdCloseFrozen', c_double),  # 卖标准平仓冻结
        ('IntraProdStdPosition', c_double),  # 品种内对冲标准持仓
        ('NetStdPosition', c_double),  # 品种内单腿标准持仓
        ('InterProdStdPosition', c_double),  # 品种间对冲标准持仓
        ('SingleStdPosition', c_double),  # 单腿标准持仓
        ('IntraProdMargin', c_double),  # 品种内对锁保证金
        ('InterProdMargin', c_double),  # 品种间对锁保证金
        ('SingleMargin', c_double),  # 跨品种单腿保证金
        ('NonCombMargin', c_double),  # 非组合合约保证金
        ('AddOnMargin', c_double),  # 附加保证金
        ('ExchMargin', c_double),  # 交易所保证金
        ('AddOnFrozenMargin', c_double),  # 附加冻结保证金
        ('OpenFrozenMargin', c_double),  # 开仓冻结保证金
        ('CloseFrozenMargin', c_double),  # 平仓冻结保证金
        ('Margin', c_double),  # 品种保证金
        ('FrozenMargin', c_double)  # 冻结保证金
    ]


class QryInvestorProdRULEMarginField(BaseField):
    """投资者产品RULE保证金查询"""
    _fields_ = [
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('BrokerID', c_char * 11),  # 经纪公司代码
...
class SyncDeltaSPBMPortfDefinitionField(BaseField):
    """风险结算追平SPBM组合保证金套餐"""
    _fields_ = [
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('PortfolioDefID', c_int),  # 组合保证金套餐代码
...
class SyncDeltaSPBMInvstPortfDefField(BaseField):
    """风险结算追平投资者SPBM套餐选择"""
    _fields_ = [
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('BrokerID', c_char * 11),  # 经纪公司代码
...
class SyncDeltaSPBMFutureParameterField(BaseField):
    """风险结算追平SPBM期货合约保证金参数"""
    _fields_ = [
        ('TradingDay', c_char * 9),  # 交易日
        ('ExchangeID', c_char * 9),  # 交易所代码
...
class SyncDeltaSPBMOptionParameterField(BaseField):
    """风险结算追平SPBM期权合约保证金参数"""
    _fields = [
        ('TradingDay', c_char * 9),  # 交易日
        ('ExchangeID', c_char * 9),  # 交易所代码
...
class SyncDeltaSPBMIntraParameterField(BaseField):
...
class SyncDeltaSPBMInterParameterField(BaseField):
    """风险结算追平SPBM跨品种抵扣参数"""
    _fields_ = [
        ('TradingDay', c_char * 9),  # 交易日
        ('ExchangeID', c_char * 9),  # 交易所代码
...
class SyncDeltaSPBMAddOnInterParamField(BaseField):
    """风险结算追平SPBM附加跨品种抵扣参数"""
    _fields_ = [
        ('TradingDay', c_char * 9),  # 交易日
        ('ExchangeID', c_char * 9),  # 交易所代码
...
class SyncDeltaSPMMInstParamField(BaseField):
    """风险结算追平SPMM合约参数"""
    _fields_ = [
        ('TradingDay', c_char * 9),  # 交易日
        ('InstrumentID', c_char * 31),  # 合约代码
...
class SyncDeltaSPMMProductParamField(BaseField):
    """风险结算追平SPMM产品相关参数"""
    _fields_ = [
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('ProductID', c_char * 41),  # 产品代码
...
class SyncDeltaInvestorSPMMModelField(BaseField):
    """风险结算追平投资者SPMM模板选择"""
    _fields_ = [
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('BrokerID', c_char * 11),  # 经纪公司代码
        ('InvestorID', c_char * 13),  # 投资者代码
        ('SPMMModelID', c_char * 33),  # SPMM模板ID
        ('ActionDirection', c_char * 1),  # 操作标志
        ('SyncDeltaSequenceNo', c_int)  # 追平序号
    ]


class SyncDeltaSPMMModelParamField(BaseField):
    """风险结算追平SPMM模板参数设置"""
    _fields_ = [
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('SPMMModelID', c_char * 33),  # SPMM模板ID
        ('CommodityGroupID', c_char * 41),  # 商品群代码
        ('IntraCommodityRate', c_double),  # SPMM品种内跨期优惠系数
        ('InterCommodityRate', c_double),  # SPMM品种间优惠系数
        ('OptionDiscountRate', c_double),  # SPMM期权优惠系数
        ('MiniMarginRatio', c_double),  # 商品群最小保证金比例
        ('ActionDirection', c_char * 1),  # 操作标志
        ('SyncDeltaSequenceNo', c_int)  # 追平序号
...
class SyncDeltaRCAMSCombProdInfoField(BaseField):
    """风险结算追平RCAMS产品组合信息"""
    _fields_ = [
        ('TradingDay', c_char * 9),  # 交易日
        ('ExchangeID', c_char * 9),  # 交易所代码
...
class SyncDeltaRCAMSInstrParameterField(BaseField):
    """风险结算追平RCAMS同合约风险对冲参数"""
    _fields_ = [
        ('TradingDay', c_char * 9),  # 交易日
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('ProductID', c_char * 41),  # 产品代码
        ('CombProductID', c_double),  # 同合约风险对冲比率
        ('ActionDirection', c_char * 1),  # 操作标志
        ('SyncDeltaSequenceNo', c_int)  # 追平序号
    ]


class SyncDeltaRCAMSIntraParameterField(BaseField):
...
class SyncDeltaRCAMSInterParameterField(BaseField):
...
class SyncDeltaRCAMSSOptAdjParamField(BaseField):
    ...
    _fields_ = [
        ...
        ('CombProductID', c_char * 41),  # 产品组合代码
        ('HedgeFlag', c_char * 1),  # 投套标志
        ('CreditRate', c_double),  # 空头期权风险调整标准
        ('ActionDirection', c_char * 1),  # 操作标志
        ('SyncDeltaSequenceNo', c_int)  # 追平序号
    ]


class SyncDeltaRCAMSCombRuleDtlField(BaseField):
    """风险结算追平RCAMS策略组合规则明细"""
    _fields_ = [
            ('TradingDay', c_char * 9),  # 交易日
            ('ExchangeID', c_char * 9),  # 交易所代码
            ('ProdGroup', c_char * 41),  # 策略产品
            ('RuleId', c_char * 51),  # 策略id
            ('Priority', c_int),  # 优先级
            ('HedgeFlag', c_double),  # 投套标志
            ('CombMargin', c_double),  # 组合保证金标准
            ('ExchangeInstID', c_char * 81),  # 交易所组合合约代码
            ('LegID', c_int),  # 单腿编号
            ('LegInstrumentID', c_char * 31),  # 单腿合约代码
            ('Direction', c_char * 1),  # 买卖方向
...
class SyncDeltaRCAMSInvstCombPosField(BaseField):
    """风险结算追平RCAMS策略组合持仓"""
    _fields_ = [
        ('ExchangeID', c_char * 9),  # 交易所代码
        ('BrokerID', c_char * 11),  # 经纪公司代码
...
class SyncDeltaRULEInstrParameterField(BaseField):
    """风险结算追平RULE合约保证金参数"""
    _fields_ = [
        ('TradingDay', c_char * 9),  # 交易日
        ('ExchangeID', c_char * 9),  # 交易所代码
...
class SyncDeltaRULEIntraParameterField(BaseField):
...
class SyncDeltaRULEInterParameterField(BaseField):
    """风险结算追平RULE跨品种抵扣参数"""
    _fields_ = [
        ('TradingDay', c_char * 9),  # 交易日
        ('ExchangeID', c_char * 9),  # 交易所代码
